...
- **1 worktree на сессию.** В начале сессии создаёшь один worktree и работаешь
  в нём до конца.
- **Правки после PR — в том же worktree.** Если PR не вмерджен и нужны доработки,
  продолжай в том же worktree — не создавай новый.
- **Новый worktree за сессию только в двух случаях:**
  1. Пользователь явно попросил создать новый.
  2. Предыдущая работа вмерджена, и семантически началась новая независимая задача.
