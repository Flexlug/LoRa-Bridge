...
- **Без ре-префикса:** текст форвардится как есть — origin-сеть уже атрибутировала
  автора (префикс/room-server author). Двойная обёртка только сожрала бы бюджет.
...
- **Ограничение (точка расширения):** цепочки/множественное членство одного
  эндпоинта в нескольких комнатах **не валидируются** — следи за топологией вручную,
  чтобы не собрать кольцо A→B→C→A. Self-loop (подписчик == первичный) запрещён схемой.
