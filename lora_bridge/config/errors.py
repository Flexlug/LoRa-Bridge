...
def format_validation_error(exc: ValidationError) -> str:
    errors = _collapse_smart_unions(list(exc.errors()))
    n = len(errors)
...
def _collapse_smart_unions(errors: list[dict]) -> list[dict]:
...
def _smart_union_variant_index(loc: tuple) -> int | None:
...
def _format_one(err: dict, n: int) -> list[str]:
    loc = err.get("loc", ())
    kind = err.get("type", "")
    inp = err.get("input")
    ctx = err.get("ctx") or {}
...
def _format_missing(header: str, loc: tuple) -> list[str]:
    ...
    if owner is not None:
        ann = owner.model_fields[field_name].annotation  # type: ignore[index]
...
def _format_extra(header: str, loc: tuple) -> list[str]:
    parent_models = _resolve_models(loc[:-1])
...
def _format_union_tag_invalid(header: str, ctx: dict) -> list[str]:
...
def _format_union_tag_not_found(header: str, ctx: dict) -> list[str]:
    disc = ctx.get("discriminator", "").strip("'\"") or "type"
    return [
        header,
...
def _format_literal(header: str, ctx: dict, inp: Any) -> list[str]:
...
def _resolve_models(loc: tuple) -> list[type[BaseModel]]:
