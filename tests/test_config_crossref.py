...
_BASE = {
    "lora": [{
        ...
        "endpoints": {"emergency": {"type": "public"}},
...
def test_valid_config_ok():
    AppConfig.model_validate(_BASE)


def test_unknown_node_rejected():
    rooms = [{"lora": {"node": "nX", "endpoint": "emergency"},
...
def test_unknown_endpoint_rejected():
    rooms = [{"lora": {"node": "n1", "endpoint": "nope"},
              "subscribers": [{"transport": "tg", "chat": "-1"}]}]
...
def test_unknown_messenger_rejected():
    rooms = [{"lora": {"node": "n1", "endpoint": "emergency"},
